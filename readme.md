...
![Sunsen_Logo](./assets/Sunsen_Logo.png)
...
| **Spring Boot**            | 自动装配、生命周期事件                                                            | `sunsen-core` 零第三方依赖（只有 JDK），通过 `ExtensionRegistrar` 接口让宿主自行对接 Spring 等容器       |
| **OSGi / PF4J**            | 依赖拓扑排序、循环依赖检测、热重载                                                      | `DependencyResolver` 做 DAG 校验和 Kahn 拓扑排序，`reloadPlugin()` 做原子替换                 |

### 1.2 几条铁律

- **契约优先**：扩展点必须是接口，插件只依赖接口，不依赖实现
- **类隔离**：插件自己的类由专属 ClassLoader 加载，公共 API（`sunsen-core` 中的 `com.bingbaihanji.sunsen.api.*`
  ）走父加载器，避免依赖冲突
- **声明优于编码**：插件身份、依赖、扩展点全写在 `plugin.json` 里，不要硬编码
- **加载和启动分开**：`@Extension` 扫描在 `LOADED` 阶段完成，`onStart()` 里才能安全地 `getExtensions()`，杜绝时序 bug
- **不绑框架**：`sunsen-core` 零第三方依赖（只有 JDK）；`sunsen-server` 仅依赖 `jackson-databind`（解析 `plugin.json`）
...
- **`sunsen-core`**：纯接口、注解、record、异常，**零外部依赖**。所有跨 ClassLoader 的类型必须在这里定义，保证类型一致性。宿主和插件都需要它
- **`sunsen-api`**：插件开发模块，依赖 `sunsen-core`。提供 `AbstractPlugin`、`ManagedScheduler`、`PluginProperties` 等开发便利类
- **`sunsen-server`**：标准实现，包括类加载器、扩展注册表、生命周期调度、依赖解析、事件总线。依赖 `sunsen-core`，唯一的外部依赖是
  `jackson-databind`（解析 `plugin.json`）
- **宿主应用**：引入 `sunsen-server`（连带 `sunsen-core`），用 `DefaultPluginManager` 管理插件生命周期
- **`{app}-api`（推荐模式）**：真实项目应将扩展点接口单独抽成一个契约模块，宿主和插件各自依赖它，互不依赖。
  `sunsen-demo-plain/demo-api` 即为此模式的示范，见下文"扩展点契约模块"一节

### 2.2 代码结构

```
sunsen/
├── sunsen-core/                       ← 公共 API，零依赖
│   ├── annotation/
│   │   ├── ExtensionPoint.java
│   │   └── Extension.java
│   ├── Plugin.java
...
└── sunsen-demo-plain/                 ← 纯 Java 演示（聚合模块）
    ├── demo-api/                      ← ① 扩展点契约接口
    │   └── Greeter / GreetingFormatter / GreetingEvent
    ├── demo-plugins/                  ← ② 三个演示插件
    │   └── src/main/java/com/.../     ← hello / world / scheduler（标准 Maven 结构）
    └── demo-host/                     ← ③ 宿主应用
        └── PlainDemoApp.java
```
...
1. **已加载的类**：直接返回
2. **公共 API 和 JDK**：`java.*`、`javax.*`、`sun.*`、`com.bingbaihanji.sunsen.api.*` 强制走父加载器，保证所有插件看到的
   `Plugin` 接口是同一个 Class 对象。这些类型定义在 `sunsen-core` 模块中
3. **插件自有类**：如果类名命中 `plugin.json` 里声明的 `packagePrefixes`，就打破双亲委派，自己加载。多个前缀时用**最长匹配**
4. **依赖插件的类**：按依赖顺序去依赖插件的 ClassLoader 里找
5. **兜底**：都找不到就委托父加载器

**前缀冲突检测**：加载前会校验所有插件的 `packagePrefixes`，如果和保留前缀（JDK、`sunsen-core` 中的
`com.bingbaihanji.sunsen.api.*`）或其他插件的前缀有包含关系，直接报错拒绝加载。
...
框架在插件进入 `LOADED` 时（`onInit()` 之后、`onStart()` 之前）自动扫描 JAR 里所有 `@Extension` 类，注册到
`ExtensionRegistry`。这意味着**第一个插件的 `onStart()` 被调用时，所有插件的扩展都已经就绪**，彻底杜绝了在 `onStart()`
里注册扩展导致的时序依赖问题。

**`allowMultiple = false`**：如果扩展点设置了 `allowMultiple = false`，`ExtensionScanner` 会在**同一插件内**
校验该扩展点的实现数量不能超过 1 个，否则加载失败。跨插件的多个实现是允许的。

**`sole = true`**：如果扩展点设置了 `sole = true`，框架会确保**整个系统**中只有一个插件的实现被注册。当多个插件都提供了实现时，框架打印警告并只保留
`@Extension(order)` 最小（优先级最高）的一个。被舍弃的实现会从注册表中移除，且即使竞争方后续卸载也不会自动恢复。适用于数据库驱动、认证后端等低频变更的基础设施选择。
...
**异常隔离**：`LifecycleStateMachine` 会捕获每个插件生命周期方法的异常，把该插件置为 `FAILED` 并发布 `PluginFailedEvent`
，不影响其他插件。
...
| `version`         | 是  | SemVer 格式                                                                   |
| `apiVersion`      | 是  | 编译时依赖的 Sunsen API 版本号, 只需 major 版本号与框架的 `SunsenVersion.API_VERSION` 一致即视为兼容 |
...
所有注解都在 `sunsen-core` 的 `com.bingbaihanji.sunsen.api.annotation` 包下。
...
对应 `plugin.json` 的内存模型，是一个 Java `record`，加载后不可变。所有集合字段都做了防御性复制（`List.copyOf` /
...
```java
// 插件 A 定义事件（放在 sunsen-core 或独立的共享模块里）
...
`PluginEventBus` 默认在发布者线程同步分发（零开销，没有线程切换）。如果需要异步，可以在构造 `DefaultPluginManager` 时传入自定义
`Executor`：
...
**并发安全**：步骤 ①–⑫ 持有写锁，`getExtensions()` 持有读锁。`ReadWriteLock` 保证读不阻塞，写操作原子完成，不存在"
扩展数量为零"的中间窗口。

**失败处理**：如果步骤 ⑦–⑬ 任意一步失败，框架会把插件置为 `FAILED` 并发布 `PluginFailedEvent`
。旧版本的扩展已经被注销了，宿主可以捕获事件后决定是否回退到旧 JAR。
...
插件访问所有框架能力（文件、配置、扩展、事件发布）的唯一通道是 `PluginContext`。宿主可以继承 `DefaultPluginContext`，在每个方法入口检查
`permissions`：
...
`ViewProvider` 是定义在 `sunsen-core` 中的扩展点接口，框架完全不感知 JavaFX 的存在。
...
| 扩展点接口放独立 `{app}-api` 模块          | 插件只依赖契约接口，不依赖宿主业务代码，实现真正解耦；`demo-api` 示范此模式      |

---

## 十一、扩展点契约模块（{app}-api 模式）

### 为什么需要独立的契约模块

宿主定义扩展点接口，插件实现它——这意味着插件需要在编译时看到接口定义。**接口放在哪里决定了耦合方向**：

| 接口放在哪里                    | 插件需要依赖什么            | 耦合度          |
|---------------------------|---------------------|--------------|
| 宿主项目（反面教材）                | 整个宿主业务模块            | 高，宿主变更影响所有插件 |
| `sunsen-core`（框架级）        | 只适合框架自带接口，业务接口不能放这里 | —            |
| **独立 `{app}-api` 模块（推荐）** | 只需轻量契约包，不依赖宿主任何实现   | 低            |

### 标准三层模块结构

```
{app}-api/        ← 扩展点接口 + 跨插件事件（只依赖 sunsen-api）
    ↑                    ↑
{app}-host/       {app}-plugins/
（宿主业务逻辑）   （插件实现）
 依赖 api + sunsen-server    依赖 api + sunsen-api
```

宿主和插件**互不依赖**，都只依赖 `{app}-api`。

### demo-api 示范

`sunsen-demo-plain/demo-api` 演示了此模式的完整实现：

```java
// demo-api 中定义——插件和宿主共同的语言
@ExtensionPoint(id = "greeter")
...
```java
// demo-plugins 中实现——只依赖 demo-api，完全不知道 demo-host 存在
@Extension(order = 10)
class EnglishGreeter implements Greeter { ... }
```

```java
// demo-host 中消费——通过接口调用，不依赖任何插件实现类
pluginManager.getExtensions(Greeter.class).forEach(g -> g.greet("Sunsen"));
```

### 在 ClassLoader 模型下的工作原理

`{app}-api` 的 JAR 在宿主 classpath 上，由父 ClassLoader 加载。`PluginClassLoader` 的加载规则保证：任何
插件中对 `Greeter.class` 的查找都会委托到父加载器，因此**所有插件和宿主看到的是同一个 `Class` 对象**，
强制类型转换不会失败。

`{app}-api` 中的类**不能**出现在任何插件的 `packagePrefixes` 中——那样会导致前缀冲突直接拒绝加载。
