...
- **类隔离**：插件自己的类由专属 ClassLoader 加载，公共 API（`sunsen-core` 中的 `com.bingbaihanji.sunsen.api.*`）走父加载器，避免依赖冲突
...
- **`sunsen-server`**：标准实现，包括类加载器、扩展注册表、生命周期调度、依赖解析、事件总线。依赖 `sunsen-core`，唯一的外部依赖是 `jackson-databind`（解析 `plugin.json`）
...
1. **已加载的类**：直接返回
2. **公共 API 和 JDK**：`java.*`、`javax.*`、`sun.*`、`com.bingbaihanji.sunsen.api.*` 强制走父加载器，保证所有插件看到的 `Plugin` 接口是同一个 Class 对象。这些类型定义在 `sunsen-core` 模块中
3. **插件自有类**：如果类名命中 `plugin.json` 里声明的 `packagePrefixes`，就打破双亲委派，自己加载。多个前缀时用**最长匹配**
4. **依赖插件的类**：按依赖顺序去依赖插件的 ClassLoader 里找
5. **兜底**：都找不到就委托父加载器

**前缀冲突检测**：加载前会校验所有插件的 `packagePrefixes`，如果和保留前缀（JDK、`sunsen-core` 中的 `com.bingbaihanji.sunsen.api.*`）或其他插件的前缀有包含关系，直接报错拒绝加载。
