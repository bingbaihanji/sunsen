...
public class ExtensionScanner {
    ...
    private static final System.Logger LOGGER = System.getLogger(ExtensionScanner.class.getName());
     ...
     * 推断扩展实现类对应的所有扩展点接口类型
     */
    static List<Class<?>> inferExtensionPointTypes(Class<?> implClass) {
    ...
    public void scan(PluginDescriptor descriptor, PluginClassLoader classLoader) {
                ...
                try (JarFile jarFile = new JarFile(jarPath.toFile())) {
                    ...
                    while (entries.hasMoreElements()) {
                        ...
                        try {
                            ...
                            // 插件 JAR 中可能包含不属于自己的 .class 文件,忽略
                        } catch (NoClassDefFoundError e) {
                            // 传递依赖缺失,扩展将丢失,需要报警以便排查
                            LOGGER.log(System.Logger.Level.WARNING,
                                    () -> "Missing transitive dependency for class " + className
                                            + " in plugin " + descriptor.id() + ": " + e.getMessage());
                        }
                    }
                }
            } catch (Exception e) {
        ...
        for (Map.Entry<Class<?>, List<Class<?>>> entry : implToEpTypes.entrySet()) {
            Class<?> implClass = entry.getKey();
            for (Class<?> epType : entry.getValue()) {
                registry.register(descriptor, implClass, epType);
            }
        }
    }
    ...
    private boolean belongsToPlugin(String className, List<String> packagePrefixes) {
        for (String prefix : packagePrefixes) {
            if (PluginClassLoader.matchesPackagePrefix(className, prefix)) {
                return true;
            }
        }
        return false;
